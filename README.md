...
* v9
The major change here is an algorithm to locate tiling in the binarised image and use this to indicate that the threshold is too low. This has fixed two cases of rat brains where the whole agar block is being imaged: the autothresh now correctly finds the brain in the block and doesn't draw a border around the agar. In the process of doing this, the four samples where we changed laser power a lot just magically work. So that's good. A new problem, however, has cropped up. Two samples that previously worked now do not. These are `HMV_JAWS_D1D2_1_2` and `191120_164248`. They now fail because the brain extends to the border to the FOV and scews the threshold. Likely these are cases that worked without rolling threshold but fail once it's added. We had to roll the threshold for other recordings to work. We need to investigate how to fix these. Possibly this will be done is tandem with [Issue #38](https://github.com/raacampbell/autofinder/issues/38)
